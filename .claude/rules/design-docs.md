...
UIに影響する変更を含むPRでは、更新した`docs/screens/<画面名>.png`をPR本文にも`<img>`タグ（`main`ブランチを指す絶対URL）で埋め込む（`.claude/skills/commit-and-pr/SKILL.md`5.1節、ADR 0006・0007）。PRごとに別のスクリーンショットを保存するアーカイブは持たない。
