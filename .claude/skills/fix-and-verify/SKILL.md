...
新規PRは作らず、`commit-and-pr`の手順で同じブランチにコミット・pushするだけで既存PRが更新されます。pushが完了したら、対応対象にした各インラインコメントに`mcp__github__add_reply_to_pull_request_comment`で返信し、何を対応したか（対応しなかった場合はその理由）を一言で書いてください。返信の先頭には必ず`[レビューイー]`を付けます（例: `[レビューイー] 対応しました（abc1234）: 日付をローカル時刻で比較するよう修正`）。スレッドのresolveは行いません（resolveは人間のレビュアーが行うものとして扱います）。再レビューはこのスキルの範囲外です（`pr-review`スキルが担当します。監視中の`pr-review`はこのpushと返信を検知して自動で再レビューします）。
...
2. イベントが届いたら内容を確認する。
   ...
   - 本文が`[レビューイー]`で始まる投稿（自分の返信の反響）、自分のpushのイベントなど、対応不要なもの → 無視する（動きとして数えない）。
   - `[レビュワー]`で始まる投稿はレビュー指摘として扱う。レビュワーとレビューイーが同じGitHubアカウントで動くことがあるため、投稿者のアカウントではなくこの見出しで見分ける。見出しのない投稿は人間のレビュアーのものとして扱い、同じく対応する。
