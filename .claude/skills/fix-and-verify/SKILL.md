---
name: fix-and-verify
description: イシュー・バグ報告・機能要望などの文章、または既存PRに付いたレビューコメント（pr-reviewスキルのmust/imo/nits指摘など）を受け取り、front/配下のコードを修正し、UIに影響する変更なら実際にアプリを起動して修正後のスクリーンショットで動作確認し、ユーザーのOKが出てから初めてcommit-and-prスキルでコミット・PR作成（既存PRへの修正なら同じブランチへpushし、各レビューコメントへ対応内容を返信）まで進める一連のワークフロー。「このイシューを直して」「バグを直してPRまで出して」「〜という不具合を修正して」「〜を実装してPR作成して」のように、修正内容の説明とPR化の両方が絡む依頼、または「直したらスクショ見せて」「動作確認してからコミットして」のように動作確認を挟みたい依頼、「PRのレビュー指摘を直して」のようにレビューコメントへの対応を頼まれた場合には必ず使うこと。単なるコミット・PR作成だけの依頼（修正は既に完了している）にはcommit-and-prスキルを直接使う。
---
...
ここで撮影したスクリーンショットのファイルは、ユーザーへの提示が終わってもこの時点では削除しないでください。UIに影響する変更の場合、ステップ5で`commit-and-pr`スキルの手順に従いPR本文へ埋め込むため、後で再利用します。
