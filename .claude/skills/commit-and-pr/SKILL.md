...
UIの見た目・挙動に影響する変更（表示の追加・変更、レイアウト、操作フローの変更など）を含むPRでは、`verify-front`スキルで撮影したスクリーンショットをPR本文に埋め込むことを必須とします。手順は次のとおりです。

1. PRを作成する（まだスクリーンショットは埋め込まなくてよい）。
2. 採番されたPR番号を使い、`docs/pr-screenshots/pr-<PR番号>/`配下にスクリーンショットのPNGを配置する（ファイル名は画面や操作内容が分かるもの、例: `index.png`、`chat.png`）。
3. これを通常のコミットとしてpushする（同じブランチへのpushなので既存PRに自動反映される）。
4. `mcp__github__update_pull_request`でPR本文の「テスト内容」セクションに、Markdown画像記法（例: `![Tasks画面](docs/pr-screenshots/pr-16/index.png)`）で埋め込む。相対パスで書けばGitHub上でそのPRのブランチを基準に描画される。

スクリーンショットはレビュー時の証跡としてリポジトリに残す運用とし、マージ後も削除しない（`docs/pr-screenshots/`はPRごとの履歴として蓄積する）。UIに影響しない変更（ロジックのみの修正など）ではこの手順は不要。
