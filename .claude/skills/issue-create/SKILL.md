...
- 調査中に依頼の範囲外の不具合や改善点を見つけた場合は、本Issueに混ぜず、別Issueの候補として手順5で提示する（勝手に起票しない）。今回のIssueの実装に影響する場合（前提になる、誤動作を引き起こすなど）は、依存関係として3.5の分割案に含める。
...
- コード変更だけでなく、次のような付随作業も漏らさずTODOに含める。
  - 仕様書の更新（`docs/screens/*.md`・スクリーンショット、`npm run generate:openapi`による`docs/api/openapi.json`の再生成など、3.1で拾ったルールに基づくもの）。ルールの`paths`に当てはまらないファイル（`app/components/`・`app/utils/`・`app/assets/css/`など）の変更でも、画面の見た目・挙動が変わる場合は`docs/screens/`の更新を含める。
...
「何ができていればこのIssueをクローズしてよいか」を、確認可能な形で箇条書きにします（例: 「空文字のタイトルでタスク追加しても一覧に増えず、エラーメッセージが表示される」）。バグ報告なら「期待する挙動」、機能要望なら「提案内容」を、確認手順に落とし込んだものになります。前半の見出しと同じ文を繰り返さず、何をすれば確認できるか（画面操作・コマンド・見るべき表示）の形で書いてください。

確認に特定のデータや条件（期限切れのタスク、特定の時刻・タイムゾーンなど）が必要な場合は、その用意の仕方も書く（シードデータに足す、`TZ=...`を付けて実行する、など）。シードデータを変える場合は、`docs/screens/`の「初期表示」の記述と`docs/demo/`のシナリオへの影響も確認する。
...
- 調査中に、依頼の実装の前提になる別の不具合が見つかった（3.1参照）

分割する場合は、まず各Issueのタイトル・範囲・依存関係（「#A のマージ後に着手」など）だけの分割案をユーザーに見せて合意を取り、その後で各Issueに手順3.2〜3.4の設計を書きます（分割を望まれなかった場合に、複数Issue分の設計が手戻りになるのを避けるため）。依頼が小さく、分割案と設計をまとめて手順5で見せたほうが早い場合はこの限りでありません。逆に、分割するとかえってIssue間の調整コストが上回る小さな変更は無理に分けず、「分割不要と判断した」と一言添えるだけでよいです。
...
- 分割したIssueの間の参照は、ドラフトでは「#（Issue A）」のような仮の表記にし、手順6で作成後の番号に置き換える。
