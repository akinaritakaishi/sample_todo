...
- 「`dev`チャンネルのToDo画面への要望を読んで、`docs/design-guidelines.md`に沿って対応して」→ 勉強会のメインシナリオ（期限切れ・今日・明日のタスクの強調表示）。シードの`dev`チャンネルに要望のやり取りが入っており、タスクのシードにも期限切れ・今日・明日・それ以外が1件ずつ入っている。進行の全体は`docs/demo/README.md`（デモ台本）を参照
